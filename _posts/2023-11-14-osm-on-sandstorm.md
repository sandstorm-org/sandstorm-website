...
But then one day on Hacker News I saw something called [Protomaps](https://protomaps.com). It's a project for vector-based tiles that render in the browser. You can have a data file in a format called pmtiles to represent a region. Then you can use [protomaps.js](https://github.com/protomaps/protomaps-leaflet) (which works with the Leaflet UI framework) that makes range requests for the specific data it needs for whatever zoom level. No need for a database, or to generate anything on the app. When I prepare the map data for download, instead of generating countless png files, I generate one pmtiles file per region.
...
The other part of the OSM experience is search. The standard option is Nominatim. It uses ElasticSearch under the hood, which again is a bit heavy duty for Sandstorm. I asked myself, "what is the sqlite of search?" It turns out the answer is... sqlite! There's a plugin called fts5 that performs reasonably well on Desert Atlas for searching names in the database (whether it can be used to handle addresses remains to be seen). To generate the search database, I decide what I want to extract(link to my script) from the raw protobuf using pyosmium(https://osmcode.org/pyosmium/) and save to a csv. When the app downloads a region, the search csv is imported into the grain's sqlite search database. Addresses, I'll still have to figure something out.

I decided that it would be useful to have cities and large towns built into the app so that the user can jump to their city by searching, and then download the region from there. For this, I used a pre-baked non-OSM database called GeoNames(http://download.geonames.org/export/dump/) as a shortcut.
